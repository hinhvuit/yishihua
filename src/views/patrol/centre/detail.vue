<!-- 
  @FileDescription:中央巡查记录详情
  @Date:2024-09-29 09:25:33
  @Author:LZW
-->

<template>
  <div class="app-container">
    <el-form class="box" :model="subParams" ref="formRef" label-width="82px" :inline="false">
      <div class="form-item">
...
<script setup name="PatrolCentreInfo">
  ...
  import { useUserType, useMeritLevel, useAccident, useCheckResult } from '@/views/dealt/patrol/useState';
  ...
  getInfoByNumberAndType({ number: route.params.number, type: 2 }).then((res) => {
    data.value = res.data;
    ...
    signData.value = res.data.patrolApproves.map((item) => {
      ...
      return {
        ...
        email: empEmail,
        state,
        applicationLevel: getUserType(2, applicationLevel)?.label,
        stopTime,
        singTime,
        meno
      };
  ...
  function goBack() {
    router.replace('/patrol/centrePatrol');
  }
