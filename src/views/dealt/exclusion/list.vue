...
<template>
  <div style="margin-bottom: 20px">
    ...
    <el-table v-loading="loading" :data="listData">
      ...
      <el-table-column label="审核状态" align="center" key="signStatus" width="120">
        <template #default="scope">
          <el-tag :type="getSign(scope.row.state).type">{{ getSign(scope.row.state).label }}</el-tag>
        </template>
      </el-table-column>
...
<script setup name="DealtList">
  ...
  const { signs, getSign, listTypes, getListType } = useState();
  let listData = ref([]);
  let total = ref(0);
  ...
  function handleInfo(row) {
    router.push(`/dealt/exclusionInfo/${row.number}?type=ExclusionAdmin`);
  }
</script>
