...
<template>
  <div class="dealt-container">
    ...
    <el-form class="box" :model="subParams" ref="formRef" label-width="82px" :inline="false" v-if="loading">
      ...
      <div class="form-item" v-if="data.status == 3">
        <div class="item-title">整改信息</div>
        <div class="item-context">
          <el-form-item
            prop="reForm.context"
            ...
            label-width="90"
          >
            <el-input v-model="subParams.reForm.context" placeholder="整改描述" type="textarea" :rows="5"></el-input>
          </el-form-item>
          <el-form-item label="相关信息：" label-width="90" style="margin-top: 20px">
            <MyUpload
              :fileSizeMax="10 * 1024"
              :fileSizeMin="0"
              :isShowTip="false"
              :fileType="['png', 'jpg', 'jpeg', 'pdf']"
              :urlList="subParams.reForm.files"
              @uploadChange="
                (file) => {
                  subParams.reForm.files = file;
                }
              "
            >
            </MyUpload
